...
export default function TasksPage() {
  ...
  const handleSaveChanges = async () => {
    ...
    if (selectedTask.status === "Completed" && editAssignee !== (selectedTask.assignedMember?._id || "")) {
        ...
        >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <ArrowUpDown size={18} style={{ color: "hsl(var(--text-secondary))" }} />
              <select
                value={sortOption}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", borderTop: "1px solid hsl(var(--border-color) / 0.5)", paddingTop: "16px" }}>
            {/* Status */}
            <select
              value={statusFilter}
            ...
            <select
              ...
              style={{
                ...
                border: "1px solid hsl(var(--border-color))",
                fontSize: "0.85rem",
                color: deadlineStatusFilter === "Overdue" ? "hsl(var(--danger))" : "inherit",
          ...
          <div className="glass-panel" style={{ overflow: "hidden" }}>
            <div className="responsive-table-container" style={{ border: "none" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid hsl(var(--border-color))", backgroundColor: "hsl(var(--bg-secondary) / 0.5)", fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>
                  ...
                  {tasks.map((task) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "16px 20px" }}>
                          {task.assignedMember ? (
                            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                              <div className="gradient-bg" style={{ width: "24px", height: "24px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "0.65rem", fontWeight: 700 }}>
                                {task.assignedMember.name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)}
                              </div>
                              <span style={{ fontSize: "0.8rem", fontWeight: 500 }}>{task.assignedMember.name}</span>
                            </div>
                          ) : (
                            <span style={{ color: "hsl(var(--text-muted))", fontSize: "0.8rem" }}>Unassigned</span>
                          )}
                        </td>
                        <td style={{ padding: "16px 20px" }}>
                        ...
                        <td style={{ padding: "16px 20px", color: overdue ? "hsl(var(--danger))" : "hsl(var(--text-secondary))", fontWeight: overdue ? 700 : "inherit" }}>
                        ...
                        <td style={{ padding: "16px 20px" }}>
                          <span className={`badge badge-${task.status === "Completed" ? "completed" : task.status === "In Progress" ? "progress" : "todo"}`}>
                            {task.status}
                          </span>
                        </td>
                      </tr>
        ...
        {isTaskModalOpen && selectedTask && (
          ...
          >
            ...
            >
              ...
              {/* Modal Body Container: Splits into Edit Form (Left) and Social Activity Feed (Right) */}
              <div style={{ display: "flex", flexGrow: 1, overflow: "hidden" }}>
                {/* Left Side: Task Fields Form */}
                <div style={{ flex: 1, padding: "32px", overflowY: "auto", borderRight: "1px solid hsl(var(--border-color) / 0.5)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", color: "hsl(var(--primary))", marginBottom: "12px", fontSize: "0.8rem", fontWeight: 700 }}>
                    <Folder size={14} />
                    <span>PROJECT: {selectedTask.project?.name || "Workspace"}</span>
                  </div>

                  <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", fontWeight: 800, marginBottom: "24px" }}>
                    {isManager ? (
                      <input
                        type="text"
                        value={editTitle}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      <label style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>Description</label>
                      {isManager ? (
                        <textarea
                          value={editDesc}
                        ...
                        <p style={{ fontSize: "0.875rem", lineHeight: 1.5, backgroundColor: "hsl(var(--bg-primary) / 0.3)", padding: "12px", borderRadius: "8px", border: "1px solid hsl(var(--border-color) / 0.5)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                      {/* Due Date */}
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        <label style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>Due Date</label>
                        {isManager ? (
                          <input
                            type="date"
                            value={editDueDate}
                          ...
                          <div style={{ padding: "10px", borderRadius: "8px", border: "1px solid hsl(var(--border-color) / 0.5)", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "8px" }}>
                            <Calendar size={14} />
                      ...
                      {/* Priority */}
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        <label style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>Priority</label>
                        {isManager ? (
                          <select
                            value={editPriority}
                          ...
                          <div style={{ padding: "10px", borderRadius: "8px", border: "1px solid hsl(var(--border-color) / 0.5)", fontSize: "0.85rem" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                      {/* Status */}
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        <label style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>Status</label>
                        <select
                        ...
                        {!canModifyStatus && (
                          <span style={{ fontSize: "0.7rem", color: "hsl(var(--danger))", marginTop: "2px" }}>
                      ...
                      {/* Assignee */}
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        <label style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))" }}>Assignee</label>
                        {isManager ? (
                          <select
                          ...
                          <div style={{ padding: "10px", borderRadius: "8px", border: "1px solid hsl(var(--border-color) / 0.5)", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "8px" }}>
                            <User size={14} />
                            <span>{selectedTask.assignedMember?.name || "Unassigned"}</span>
                          </div>
                        )}
                  ...
                  <div style={{ borderTop: "1px solid hsl(var(--border-color) / 0.5)", paddingTop: "20px", marginTop: "32px", display: "flex", justifyContent: "flex-end" }}>
                    <button
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
                  {/* Tabs headers */}
                  <div style={{ display: "flex", borderBottom: "1px solid hsl(var(--border-color) / 0.5)", backgroundColor: "hsl(var(--bg-secondary) / 0.3)" }}>
                    <div style={{ flex: 1, padding: "16px", textTransform: "uppercase", fontSize: "0.75rem", fontWeight: 700, letterSpacing: "0.5px", textAlign: "center", borderBottom: "2px solid hsl(var(--primary))", color: "hsl(var(--primary))" }}>
                  ...
                  <div style={{ flexGrow: 1, overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
                    ...
                    <div>
                      <h4 style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))", marginBottom: "12px", textTransform: "uppercase" }}>File Assets</h4>
                      {selectedTask.attachments?.length === 0 ? (
                        <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-muted))" }}>No shared attachments yet.</p>
                      ) : (
                      ...
                      <form onSubmit={handleAddAttachment} style={{ display: "flex", gap: "8px", marginTop: "10px" }}>
                        <input
                          type="text"
                          required
                    ...
                    <div style={{ height: "1px", backgroundColor: "hsl(var(--border-color) / 0.5)", margin: "8px 0" }} />

                    {/* Comments Feed list */}
                    <div>
                      <h4 style={{ fontSize: "0.8rem", fontWeight: 700, color: "hsl(var(--text-secondary))", marginBottom: "12px", textTransform: "uppercase" }}>Discussion Thread</h4>
                      {selectedTask.comments?.length === 0 ? (
                        <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-muted))", textAlign: "center", padding: "20px 0" }}>No comments yet. Start the conversation below!</p>
                      ) : (
                        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                          {selectedTask.comments.map((comm, idx) => (
                            <div key={idx} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
                              <div className="gradient-bg" style={{ width: "28px", height: "28px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "0.7rem", fontWeight: 700, flexShrink: 0 }}>
                                {comm.user?.name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)}
                              </div>
                              <div style={{ flexGrow: 1, padding: "10px 12px", borderRadius: "8px", backgroundColor: "hsl(var(--bg-primary) / 0.4)", border: "1px solid hsl(var(--border-color) / 0.3)" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                  <span style={{ fontSize: "0.75rem", fontWeight: 700 }}>{comm.user?.name} ({comm.user?.role.replace("_", " ")})</span>
                                  <span style={{ fontSize: "0.65rem", color: "hsl(var(--text-muted))" }}>
                                    {new Date(comm.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                  </span>
                                </div>
                                <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-primary))", lineHeight: 1.4 }}>{comm.text}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <form onSubmit={handleAddComment} style={{ padding: "20px 24px", borderTop: "1px solid hsl(var(--border-color) / 0.5)", display: "flex", gap: "12px", backgroundColor: "hsl(var(--bg-secondary) / 0.5)" }}>
                    <input
                      type="text"
